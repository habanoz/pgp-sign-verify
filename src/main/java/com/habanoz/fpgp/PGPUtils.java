...
public class PGPUtils {
	...
	static BcKeyFingerprintCalculator bskf = new BcKeyFingerprintCalculator();

	@SuppressWarnings("unchecked")
	public static PGPPublicKey readPublicKey(InputStream in)
			throws IOException, PGPException {

		PGPPublicKeyRingCollection keyRingCollection = new PGPPublicKeyRingCollection(PGPUtil.getDecoderStream(in), bskf);
	...
	public static PGPSecretKey readSecretKey(InputStream in)
		...
		PGPSecretKeyRingCollection keyRingCollection = new PGPSecretKeyRingCollection(PGPUtil.getDecoderStream(in), bskf);
	...
	public static PGPPrivateKey findPrivateKey(InputStream keyIn, long keyID, char[] pass)
			throws IOException, PGPException {
		PGPSecretKeyRingCollection pgpSec = new PGPSecretKeyRingCollection(PGPUtil.getDecoderStream(keyIn), bskf);
		return findPrivateKey(pgpSec.getSecretKey(keyID), pass);
	}

	public static PGPPrivateKey findPrivateKey(File privateKeyFile, long keyID, char[] pass)
			throws IOException, PGPException {
		try (InputStream in = new FileInputStream(privateKeyFile)) {
			return findPrivateKey(in, keyID, pass);
		}
	}
	...
	public static void decryptFile(InputStream in, OutputStream out, InputStream keyIn, char[] passwd)
		...
		PGPObjectFactory pgpF = new PGPObjectFactory(in, bskf);
		...
		PGPObjectFactory plainFact = new PGPObjectFactory(clear, bskf);

		Object message = plainFact.nextObject();

		if (message instanceof PGPCompressedData) {
			PGPCompressedData cData = (PGPCompressedData) message;
			PGPObjectFactory pgpFact = new PGPObjectFactory(cData.getDataStream(), bskf);
	...
	/**
	 * [pgp-package] -> [encrypted-session-key],[encrypted-signed-data]
	 * [encrypted-session-key] -> [encrypted-session-key] + rsa + password + private key of decryptor -> [session-key]
	 * [encrypted-signed-data] -> [encrypted-signed-data] + triple-des + session key -> [data], [signature]
	 * [signature] -> [signature] + rsa + public key of signer -> [decrypted data hash]
	 * [data] -> hashing algorithm -> [calculated data hash] must equal to [decrypted data hash]
	 *
	 * @param inputFile               file to decrypt and verify
	 * @param outputFile              file to write decrypted content, if not specified decrypted content is not written
	 * @param decryptorPrivateKeyFile file containing private key of the decryptor
	 * @param signerPulicKeyFile      file containing public key of the signer
	 * @param passwd                  password fot private key
	 * @return true if verification is successful. Hash found in message must be equal to hash calculated using decrypted data
	 * @throws Exception
	 */
	@SuppressWarnings("unchecked")
	public static boolean decryptSignedFile(File inputFile, File outputFile, File decryptorPrivateKeyFile, File signerPulicKeyFile, char[] passwd)
			throws Exception {
		Security.addProvider(new BouncyCastleProvider());

		InputStream inputFileStream = new FileInputStream(inputFile);
		InputStream inputStream = org.bouncycastle.openpgp.PGPUtil.getDecoderStream(inputFileStream);

		PGPObjectFactory pgpF = new PGPObjectFactory(inputStream, bskf);
		PGPEncryptedDataList enc;
		...
		//
		Iterator<PGPPublicKeyEncryptedData> it = enc.getEncryptedDataObjects();
		PGPPrivateKey decryptorPrivateKey = null;
		PGPPublicKeyEncryptedData pbe = null;

		while (decryptorPrivateKey == null && it.hasNext()) {
			pbe = it.next();

			decryptorPrivateKey = findPrivateKey(decryptorPrivateKeyFile, pbe.getKeyID(), passwd);
		}
		...
		InputStream clear = pbe.getDataStream(new BcPublicKeyDataDecryptorFactory(decryptorPrivateKey));
		...
		if (message instanceof PGPOnePassSignatureList) {
			...
			try (InputStream signerPulicKeyIn = new FileInputStream(signerPulicKeyFile)) {
				pgpRing = new PGPPublicKeyRingCollection(PGPUtil.getDecoderStream(signerPulicKeyIn), bskf);
			}

			PGPPublicKey signerPublicKey = pgpRing.getPublicKey(ops.getKeyID());
			...
			// only write to output, if outputfile is set
			OutputStream outputStream = null;
			if (outputFile != null)
				outputStream = new FileOutputStream(outputFile);

			// calculate file hash and write decrypted output
			try (InputStream dIn = p2.getInputStream()) {
				while ((ch = dIn.read()) >= 0) {
					ops.update((byte) ch);
	...
	/**
	 * [data] -> [data] + hashing algorithm -> [data hash]
	 * [data hash]->[data hash] + private key of signer -> [signature]
	 * ([data],[signature]) ->  [signed-data]
	 * [signed-data] -> [signed-data] + [session-key] + triple-des -> [encrypted-signed-data]
	 * [session-key] -> [session-key] + rsa + public key of decryptor/receiver -> [encrypted-session-key]
	 * [encrypted-session-key],[encrypted-signed-data] -> [pgp-package]
	 *
	 * @param signedEncryptedFile generate file
	 * @param inputFile           file to sign and ecrypt
	 * @param publicKey           public key of decryptor/receiver
	 * @param secretKey           secret key of signer
	 * @param password            password for secret key of signer
	 * @param armor
	...
	public static void signEncryptFile(
		...
		OutputStream fileOutputStream = new FileOutputStream(signedEncryptedFile);
		OutputStream outputStream = fileOutputStream;
		if (armor) {
			outputStream = new ArmoredOutputStream(outputStream);
		}
		OutputStream encryptedOut = encryptedDataGenerator.open(outputStream, new byte[PGPUtils.BUFFER_SIZE]);
	...
	private static void copyFileUsingStream(InputStream source, OutputStream dest) throws IOException {
